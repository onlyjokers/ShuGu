...
export class ScreenController {
  ...
  setColor(payload: ScreenColorPayload): void {
    ...
    const blinkFrequency = Number.isFinite(payload.blinkFrequency)
      ? (payload.blinkFrequency as number)
      : 2;
    const pulseDuration = Number.isFinite(payload.pulseDuration)
      ? (payload.pulseDuration as number)
      : 1200;
    const pulseMin = this.clamp(payload.pulseMin ?? payload.minOpacity ?? 0.25, 0, 1);
    const cycleDuration = Number.isFinite(payload.cycleDuration)
      ? (payload.cycleDuration as number)
      : 4000;
    const cycleColors =
...
export class ToneSoundPlayer {
  ...
  async update(
    ...
    const nextVolume =
      payload.volume !== undefined
        ? this.clamp(Number(payload.volume), 0, 1)
        : (this.lastVolume ?? 1);
    const nextLoop = payload.loop !== undefined ? Boolean(payload.loop) : (this.lastLoop ?? false);
    const fadeInMs =
      payload.fadeIn !== undefined ? Number(payload.fadeIn) : (this.lastFadeInMs ?? 0);
    ...
    if (toneAudioEngine.isEnabled()) {
      ...
      try {
        ...
        if (Tone && gainParam?.setValueAtTime) {
          const now = Tone.now();
          gainParam.setValueAtTime(nextVolume, now);
        }
      } catch {
        // ignore
      }
  ...
  private async playTone(
    ...
    const player = new Tone.Player({ url, loop: opts.loop, autostart: false });
    player.connect(gain);
  ...
  private async playHtml(
    ...
    try {
      ...
      if (raw && raw.createMediaElementSource) {
        ...
        const destInput = (Tone as unknown as { Destination?: { input?: AudioNode } }).Destination
          ?.input;
        if (destInput) {
...
export class ToneModulatedSoundPlayer {
  ...
  async play(payload: ModulateSoundPayload, delaySeconds = 0): Promise<void> {
    ...
    const volume = this.clampNumber(payload.volume, 0, 1, 0.7);
    const freq = this.clampNumber(payload.frequency, 0, 40000, 180);
    const waveform =
      payload.waveform === 'sine' ||
      payload.waveform === 'square' ||
      payload.waveform === 'triangle' ||
      payload.waveform === 'sawtooth'
        ? payload.waveform
        : 'square';
    const modDepth = this.clampNumber(payload.modDepth, 0, 1, 0);
    const modFreq = this.clampNumber(payload.modFrequency, 0, 200, 12);

    this.startAtSeconds = now;
    this.durationSeconds = duration;
    this.releaseSeconds = release;

    this.gain = new Tone.Gain(0).toDestination() as unknown as ToneGainLike;
    this.carrier = new Tone.Oscillator({
      frequency: freq,
      type: waveform,
    }) as unknown as ToneOscillatorLike;
    this.carrier.connect(this.gain as unknown as AudioNode);
    ...
    if (modDepth > 0) {
      ...
      this.lfo = new Tone.LFO({
        ...
        amplitude: 1,
        units: 'number',
      }) as unknown as ToneLfoLike;
      const freqParam = this.carrier.frequency;
      if (freqParam) {
        this.lfo.connect(freqParam as unknown as AudioParam);
      }
      this.lfo.start?.(now);
    }
  ...
  async update(payload: {
    ...
    // Update LFO
    if (
      payload.modDepth !== undefined ||
      payload.modFrequency !== undefined ||
      payload.frequency !== undefined
    ) {
      const carrierFreqParam = this.carrier.frequency;
      const currentFreq =
        payload.frequency !== undefined
          ? payload.frequency
          : this.clampNumber(Number(carrierFreqParam?.value ?? 0), 0, 40000, 0);

      const modDepth =
        payload.modDepth !== undefined ? this.clampNumber(payload.modDepth, 0, 1, 0) : null;
      ...
      if (!this.lfo && modDepth !== null && modDepth > 0) {
        ...
        if (carrierFreqParam) {
          this.lfo.connect(carrierFreqParam as unknown as AudioParam);
        }
        this.lfo.start?.(now);
      }
