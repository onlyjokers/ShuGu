...
function createClientCountNode(deps) {
    return {
        type: 'client-count',
        label: 'Client Count',
        category: 'Objects',
...
function createArrayFilterNode() {
    return {
        type: 'array-filter',
        label: 'Array Filter',
        category: 'Logic',
...
function mergeImageHashParam(baseRef, key, value) {
    if (!baseRef)
        return '';
...
function createImgXYOffsetNode() {
    return {
        type: 'img-xy-offset',
        label: 'Img XY Offset',
        category: 'Image',
...
function createImgTransparencyNode() {
    return {
...
function createClientObjectNode(deps) {
    return {
        ...
        process: (inputs, config, context) => {
            ...
            const imageOut = typeof deps.getImageForClientId === 'function' && primaryClientId
...
const showImageCommandCache = new Map();
function createShowImageProcessorNode() {
    const resolveUrl = (raw) => {
        if (typeof raw === 'string')
            return raw.trim();
        if (Array.isArray(raw)) {
...
function createSceneBoxNode() {
    const coerceSceneChain = (raw) => (Array.isArray(raw) ? raw : []).filter((v) => Boolean(v) && typeof v === 'object' && typeof v.type === 'string');
    return {
        type: 'scene-box',
        label: 'Scene Box',
        category: 'Scene',
        ...
        process: (inputs) => {
            ...
            const scene = { type: 'box' };
            return { out: [...chain, scene] };
        },
    };
}
function createSceneMelNode() {
    const coerceSceneChain = (raw) => (Array.isArray(raw) ? raw : []).filter((v) => Boolean(v) && typeof v === 'object' && typeof v.type === 'string');
    return {
        type: 'scene-mel',
        ...
        process: (inputs) => {
            const chain = coerceSceneChain(inputs.in);
            const scene = { type: 'mel' };
            return { out: [...chain, scene] };
        },
    };
}
function createSceneFrontCameraNode() {
    const coerceSceneChain = (raw) => (Array.isArray(raw) ? raw : []).filter((v) => Boolean(v) && typeof v === 'object' && typeof v.type === 'string');
    return {
        ...
        process: (inputs) => {
            const chain = coerceSceneChain(inputs.in);
            const scene = { type: 'frontCamera' };
            return { out: [...chain, scene] };
        },
    };
}
function createSceneBackCameraNode() {
    const coerceSceneChain = (raw) => (Array.isArray(raw) ? raw : []).filter((v) => Boolean(v) && typeof v === 'object' && typeof v.type === 'string');
    return {
